...
export const voteRouter = router({
  ...
  toggle: onboardedProcedure.input(voteToggleInput).mutation(async ({ ctx, input }) => {
    ...
    const targetDone = input.done ?? !(existing?.done ?? false);
  ...
  week: onboardedProcedure.input(voteWeekInput).query(async ({ ctx, input }) => {
    ...
    const rows = identities.map((id) => {
      ...
      const slabs = days.map((d) => {
        ...
        if (!scheduled) return { date: d, state: "na" as const, isToday: d === today };
        if (done) {
          totalVotes += 1;
          totalScheduled += 1;
          return { date: d, state: "done" as const, isToday: d === today };
        }
        totalScheduled += 1;
        if (d < today) return { date: d, state: "miss" as const, isToday: false };
        return { date: d, state: "pending" as const, isToday: d === today };
      });
